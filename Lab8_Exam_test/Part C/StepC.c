...
unsigned short int binaryToThermo (unsigned short int);
...
void main() {
    ...
    while (1){
        ...
        if (adc_ready && (sonar_ready == 0)){
            ...
            // LATD = binaryToThermo(ADRESH);
            strcpy(txt,"ADC[mV] =");
            IntToStr((int)adc_res*5, num_txt);
            strcat(txt, num_txt);
            Lcd_Out(2,1,txt);
            // IntToStr((int)ADRESH, num_txt);
            // Lcd_Out(1,1,num_txt);
            adc_ready = 0;
            ADCON0.GO_NOT_DONE = 1;
        }
...
unsigned short int binaryToThermo (unsigned short int binary) {
    ...
    /*
    0       - 0b000 00000
    31      - 0b000 11111              0b0000 0001 = 2^1-1

    32      - 0b001 00000
    63      - 0b001 11111              0b0000 0011 = 2^2-1

    64      - 0b010 00000
    95      - 0b010 11111              0b0000 0111 = 2^3-1

    96      - 0b011 00000
    127     - 0b011 11111              0b0000 1111 = 2^4-1
    ...
    Check the 3 MSB of each group. They define the group!
    The thermometric number is given by 2^(3MSB+1)-1.
    */ 
    thermo = (1<<((binary>>5)+1))-1;
    return thermo;

    //thermo = (binary>0) | ((binary>32)<<1) |...
}
